 ...
 * registers that represent this process. Look up what these registers do in x86.
*/
struct jobctx{
  uint64_t rsp;
  uint64_t r15;
...
/**
 * Called when a new job arrives. You should store the scheduler_info data inside newjob->metadata 
 ...
 */
void scheduler_new_job(job *newjob, int job_number, double time, scheduler_info *stats); 
 ...
 * You should return the job* of the next thread to be run. If there are no waiting
 * threads, return NULL. 
 *  
 * Note that if you are using a non-preemptive scheme this function should
 * return job_evicted if it is not NULL. 
 * 
 * @param job_evicted job that is currently running (NULL if no jobs are running) 
 * @param time the current time.
 ...
 *
 * Assumptions:
 *   - This function will only be called after all jobs that have arrived have completed 
