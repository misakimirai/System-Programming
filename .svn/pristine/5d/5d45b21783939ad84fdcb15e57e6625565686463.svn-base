...
static int load_shared_file_test(void) {
    ...
    // syscall_mmap, you'll fail because of this
    char const *red_herring = "tmp/red_herring";
    FILE *tmpfp2 = fopen(red_herring, "w+");
    obtain_mapped_file(red_herring);
    fclose(tmpfp2);
...
static int page_out_shared_file_test(void) {
    ...
    // Process 2's PTE 1 needs to be present and dirty, and all others not
