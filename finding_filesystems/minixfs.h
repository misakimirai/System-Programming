...
typedef struct {
  ...
  /* Curious about how this is a variable sized bitmap?
 ...
 *      NUM_INDIRECT_INODES) fail without writing any data and set errno to ENOSPC
 *
 ...
 *
 *  After reading successfully, *off should be incremented by the number of bytes read.
 * 
...
/*
 * This function will first check if `path` exists. If it does, it will ensure that the 
 * inode that represents `path` has at least `block_count` many data blocks
 * including both direct and indirect data blocks. If that file already has `block_count` 
 * blocks, 0 will be retunred. If it does not, it will allocate blocks for it.
 *
...
/*
 *  This function is very similar to stat(2). It will populate the passed in buffer
 *  with the file's attributes.
 */
int minixfs_stat(file_system *fs, char *paths, struct stat *buf);

/*
*  Creates a file located at path, return NULL if inode
*    Already exists
*/
...
/*
 *  This function reads all directory entries from a directory and stores them in
