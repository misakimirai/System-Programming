...
int parse_header (connection_state *curr_state, char *buffer, size_t count) {
  ...
  // printf("The header_count is %zu at line %d\n", curr_state->header_count, __LINE__);
  memmove(curr_state->header + curr_state->header_count, buffer, count);
  curr_state->header_count += count;
  ...
  // int new_line = 0;
  // int i = 0;
  // for (; i < (int)curr_state->header_count; i++) {
  ...
  int blank = 0;
  for (size_t i = 0; i < curr_state->offset; i++) {
    if (curr_state->header[i] == ' ') {
      blank++;
    }
  }
  ...
  if (!strncmp(curr_state->header, "PUT ", 4)) {
  ...
  else if (!strncmp(curr_state->header, "LIST\n", 5)) {
...
void initialize(int fd) {
  ...
  new_state->header = calloc(1, 4096);
...
void handle_data(int fd) {
  ...
  while(1) {
  	char *buff = calloc(1, 1024);
		ssize_t count = read(fd, buff, 1023);
		if(count == -1) {
