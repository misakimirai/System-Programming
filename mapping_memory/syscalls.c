...
void print_flags(struct vm_area* v) {
   fprintf(stderr, "************************************************\n");
   fprintf(stderr, "prot_read: %u, prot_write: %u, prot_exec: %u\n", v->prot_read, v->prot_write, v->prot_exec);
   fprintf(stderr, "map_private: %u, map_anonymous: %u\n", v->map_private, v->map_anonymous);
   fprintf(stderr, "************************************************\n");
}
...
uint32_t syscall_mmap(uint32_t length, int prot_flags, int flags,
   ...
   uint32_t share = !!(flags & MAP_SHARED);
   uint32_t private = !!(flags & MAP_PRIVATE);
   uint32_t anno = !!(flags & MAP_ANONYMOUS);

   if(!(share ^ private) || (share && anno))
      return MAP_FAILED;

   if(length % PAGE_SIZE || offset % PAGE_SIZE)
      return MAP_FAILED;

   struct mapped_file *f = NULL;
   if(!anno) {
      f = obtain_mapped_file(pathname);
      if(NULL == f)
         return MAP_FAILED;
   }
   ...
   list_rev_for_each(current->stack_segment, current->heap_segment, area_prev, area_next, elem) {
      ...
      uint32_t gap = elem->start_addr - prev->end_addr;
      if(gap >= length) {
         struct vm_area *vma = NULL;
         vma = create_vm_area(current, elem->start_addr - length, elem->start_addr);
         if(NULL == vma)
            return MAP_FAILED;

         vma->prot_read = !!(prot_flags & PROT_READ);
         vma->prot_write = !!(prot_flags & PROT_WRITE);
         vma->prot_exec = !!(prot_flags & PROT_EXEC);
         vma->map_anonymous = anno;
         vma->map_private = private;
         vma->vm_mapped_next = NULL;
         vma->vm_mapped_prev = NULL;
         ...
         list_link_before(vma, elem->area_prev, area_prev, area_next);

         if(share)
            list_link_before(vma, &vma->file->shared_vma_list, vm_mapped_prev, vm_mapped_next);
...
addr32 syscall_sbrk(uint32_t shift) {
   shift = page_align(shift);
   struct vm_area *heap = current->heap_segment;
   if(heap->end_addr + shift > heap->area_next->start_addr)
      return MAP_FAILED;
   heap->end_addr += shift;
