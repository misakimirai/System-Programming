...
void mmu_read_from_virtual_address(mmu *this, addr32 virtual_address,
    ...
    // check the invalid memory address
    if (!address_in_segmentations(this->segmentations[pid], virtual_address)) {
      mmu_raise_segmentation_fault(this);
      return;
    }
    uintptr_t vpn1 = virtual_address >> 22;
    uintptr_t vpn2 = (virtual_address << 10) >> 22;
    uintptr_t offset = (virtual_address << 20) >> 20;
    uintptr_t base_virtual_addr = virtual_address >> 12;
    page_table_entry *entry = tlb_get_pte(&(this->tlb), base_virtual_addr);

    if (!entry) {
      mmu_tlb_miss(this);
      page_directory *my_pd = this->page_directories[pid];
      if (!my_pd) {
        mmu_add_process(this, pid);
        my_pd = this->page_directories[pid];
      }
      page_directory_entry *my_entry = (page_directory_entry*)my_pd + vpn1;
      if (!my_entry->present) {
        mmu_raise_page_fault(this);
        my_entry->base_addr = ((uintptr_t)ask_kernel_for_frame((page_table_entry*)my_entry)) >> 12;
        read_page_from_disk((page_table_entry*)my_entry);
        my_entry->user_supervisor=1;
        my_entry->read_write=1;
        my_entry->present = 1;
      }
      page_table *my_pt = (page_table*)(((uintptr_t)my_entry->base_addr) << 12);
      entry = my_pt->entries + vpn2;
      tlb_add_pte(&(this->tlb), base_virtual_addr, entry);
    }
    if (!entry->present) {
      mmu_raise_page_fault(this);
      entry->base_addr = ((uintptr_t)ask_kernel_for_frame(entry)) >> 12;
    ...
    entry->accessed = 1;
    uintptr_t physical_memory = (((uintptr_t)entry->base_addr) << 12) + offset;
    memcpy(buffer, (void*)physical_memory, num_bytes);
}
...
void mmu_write_to_virtual_address(mmu *this, addr32 virtual_address, size_t pid,
    ...
    entry->accessed = 1;
    uintptr_t physical_memory = (((uintptr_t)entry->base_addr) << 12) + offset;
    memcpy((void*)physical_memory, buffer, num_bytes);
}
